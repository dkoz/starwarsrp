	...
	if LocalPlayer():IsAdmin() or LocalPlayer():IsSuperAdmin() then
		...
		menu:AddOption( "Kick", function()
			...
			kicksubmit.DoClick = function()
				RunConsoleCommand( "sam", "kick", ply:Nick(), kickinput:GetValue() )
				kickframe:Remove()
			end
		...
		menu:AddOption( "Slay", function() RunConsoleCommand( "sam", "slay", ply:Nick() ) end ):SetIcon( "icon16/cross.png" )
		menu:AddOption( "Ban", function()
			...
			bansubmit.DoClick = function()
				RunConsoleCommand( "sam", "ban", ply:Nick(), bantime:GetValue(), baninput:GetValue() )
				banframe:Remove()
			end
		...
		jailmenu:AddOption( "Jail", function() RunConsoleCommand( "sam", "jail", ply:Nick() ) end )
		jailmenu:AddOption( "Unjail", function() RunConsoleCommand( "sam", "unjail", ply:Nick() ) end )
		
		local mutemenu, icon = menu:AddSubMenu( "Mute" )
		icon:SetIcon( "icon16/sound.png" )
		mutemenu:AddOption( "Mute", function() RunConsoleCommand( "sam", "mute", ply:Nick() ) end )
		mutemenu:AddOption( "Unmute", function() RunConsoleCommand( "sam", "unmute", ply:Nick() ) end )
		
		local teleportmenu, icon = menu:AddSubMenu( "Teleport" )
		icon:SetIcon( "icon16/arrow_right.png" )
		teleportmenu:AddOption( "Goto", function() RunConsoleCommand( "sam", "goto", ply:Nick() ) end )
		teleportmenu:AddOption( "Bring", function() RunConsoleCommand( "sam", "bring", ply:Nick() ) end )
	end
	menu:Open()
end